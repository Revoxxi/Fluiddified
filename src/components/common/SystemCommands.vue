<template>
  <div>
    ...
    <v-list-group
      v-if="devicePowerComponentEnabled && powerDevices.length && canControlPowerDevices"
      prepend-icon="$power"
      no-action
    >
      <template #activator>
...
@Component({})
export default class SystemCommands extends Mixins(StateMixin, ServicesMixin, AuthMixin) {
  get canRestartServices (): boolean {
    return this.hasPermission(Permissions.SYSTEM_RESTART)
  }

  get canControlPowerDevices (): boolean {
