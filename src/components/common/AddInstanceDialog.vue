<template>
  ...
  >
    <v-card-text>
      ...
      <v-text-field
        v-if="hosted"
        v-model="proxyBackendId"
        class="mt-2"
        spellcheck="false"
        clearable
        :label="$t('app.endpoint.label.proxy_backend_id')"
        persistent-hint
        :hint="$t('app.endpoint.hint.proxy_backend_id')"
      />
...
export default class AddInstanceDialog extends Mixins(StateMixin) {
  ...
  @Watch('open')
  onDialogOpen (opened: boolean | undefined) {
    if (opened) {
      this.proxyBackendId = ''
    }
  }
  ...
  addInstance () {
    ...
    if (this.hosted && this.proxyBackendId.trim()) {
      apiConfig.proxyBackendId = this.proxyBackendId.trim()
    }
    this.open = false
