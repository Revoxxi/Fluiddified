<template>
  ...
  >
    ...
    <div class="toolbar-supplemental">
      <div v-if="socketConnected && uiSessionActive && klippyReady && showSaveConfigAndRestartForPendingChanges">
      ...
      <div v-if="socketConnected && uiSessionActive && !isMobileViewport && canEmergencyStop">
        <v-tooltip bottom>
      ...
      <div v-if="socketConnected && uiSessionActive && topNavPowerToggle && canControlTopNavPower">
        <v-tooltip bottom>
          <template #activator="{ on, attrs }">
            <app-btn
              icon
      ...
      <div v-if="socketConnected && uiSessionActive">
...
export default class AppBar extends Mixins(StateMixin, ServicesMixin, FilesMixin, BrowserMixin, AuthMixin) {
  menu = false
  ...
  get canControlTopNavPower (): boolean {
    return this.hasPermission(Permissions.SYSTEM_POWER)
  }
