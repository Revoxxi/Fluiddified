  ...
  >
    ...
    >
      ...
      >
        ...
        <div
          v-if="socketConnected && showMainNavItems"
          class="nav-items"
        >
          <app-nav-item
            icon="$dash"
        ...
        <template
          v-if="socketConnected && showMainNavItems && !isMobileViewport && canEditLayout && hasRouteAccess('user')"
          #append
        >
          <v-tooltip right>
...
export default class AppNavDrawer extends Mixins(StateMixin, BrowserMixin) {
  ...
  get showMainNavItems (): boolean {
    return this.$route.name !== 'login' && this.uiSessionActive
  }

  get showSubNavigation () {
    return this.hasSubNavigation && this.socketConnected && this.showMainNavItems
  }
  ...
  hasRouteAccess (minRole: Role): boolean {
    return this.$typedGetters['auth/hasMinRole'](minRole)
  }
}
</script>
