...
export const setSocketActionsStore = (store: Store<RootState>) => {
  _store = store
}

const checkMethodPermission = (method: string): boolean => {
  ...
  if (RoleHierarchy[currentRole] >= RoleHierarchy[minRole]) return true

  consola.warn(`Permission denied: ${method} requires ${minRole}, current role is ${currentRole}`)
  EventBus.$emit('Permission denied', { type: 'warning', timeout: 3000 })
  return false
}
...
const baseEmit = async <T = unknown>(method: string, options: NotifyOptions): Promise<T> => {
  ...
  if (!checkMethodPermission(method)) {
