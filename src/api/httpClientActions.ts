...
export const httpClientActions = {
  ...
  accessUsersListGet (options?: AxiosRequestConfig) {
    return this.get<{
      result: Moonraker.Authorization.UsersListResponse
    }>('/access/users/list', options)
  },

  accessPostUserCreate (username: string, password: string, options?: AxiosRequestConfig) {
    return this.post<{
      result: Moonraker.Authorization.PostUserResponse
    }>('/access/user', {
      username,
      password
    }, options)
