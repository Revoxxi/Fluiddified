...
const createHttpClient = (store: Store<RootState>) => {
  ...
  const responseInterceptor = (response: AxiosResponse) => {
    switch (response.status) {
      case 200:
      // Only treat as "logged in" when a Bearer token was sent — trusted clients can get
      // 200s from Moonraker without JWT; those must not set `authenticated` or login UI breaks.
        if (
  ...
  const errorInterceptor = async (error: AxiosError<string | { error?: { message?: string } } | undefined>) => {
    ...
    switch (error.response.status) {
      ...
      case 400:
        ...
        break
      case 401:
        // Optional LAN sessions may call withAuth routes without a JWT; do not nuke session.
        if (error.config?.withAuth && store.state.auth.token) {
