...
function normalizeBackend (raw: Record<string, unknown>, index: number): ProxyBackend | null {
...
function parseInstancesFromEnv (): ProxyBackend[] | undefined {
  ...
  try {
    ...
    const out: ProxyBackend[] = []
    for (let i = 0; i < parsed.length; i++) {
      const row = parsed[i]
      if (!row || typeof row !== 'object') continue
      const b = normalizeBackend(row as Record<string, unknown>, i)
      if (b) out.push(b)
    }
...
function defaultSingleBackend (): ProxyBackend {
  return {
