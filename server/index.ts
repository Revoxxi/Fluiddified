...
async function validateMoonrakerConfig (moonrakerHost: string, moonrakerPort: number) {
  try {
    const res = await fetch(
      `http://${moonrakerHost}:${moonrakerPort}/server/config`
    )
    const data = await res.json() as {
      result?: { config?: { authorization?: { trusted_clients?: string[], force_logins?: boolean } } }
    }
...
async function start () {
  ...
  for (const b of config.instances) {
    console.log(
      `[server] Backend ${b.id}: Moonraker ${b.moonrakerHost}:${b.moonrakerPort}` +
